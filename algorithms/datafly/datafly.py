...
class Datafly(AbstractAlgorithm):
    ...
    def generate_initial_partitions(self):
        ...
        for attr_name, value in self.numerical_attr_config.items():
            ...
            new_partition_accumulator: list[dict[str, Attribute]] = []
            for num_range in num_ranges:
                new_partition_accumulator += self.combine_attribute_with_existing_partitions(temp_partitions, attr_name, num_range)

            temp_partitions = new_partition_accumulator
        ...
        for attr_name, value in self.categorical_attr_config.items():            
            ...
            new_partition_accumulator: list[dict[str, Attribute]] = []
            for node in nodes:
                new_partition_accumulator += self.combine_attribute_with_existing_partitions(temp_partitions, attr_name, node)
